...
export default function AdminWidgetsPage() {
  ...
  const toggle = (key: string, plan: string, type: "plan" | "tipo") => {
    setWidgets(ws => ws.map(w => {
      if (w.key !== key) return w;
      const list = (type === "plan" ? w.plans : w.tipos) || [];
      const exists = list.includes(plan);
      const updated = exists ? list.filter(p => p !== plan) : [...list, plan];
  ...
  if (loading || !widgets.length) return <div className="p-4"><Spinner /></div>;
            ...
            <tr key={w.key}>
              ...
              {PLANS.map(p => (
                <td key={p} className="text-center">
                  <input type="checkbox" checked={w.plans?.includes(p) ?? false}
                    onChange={() => toggle(w.key, p, "plan")}/>
                </td>
              ))}
              {TYPES.map(t => (
                <td key={t} className="text-center">
                  <input type="checkbox" checked={w.tipos?.includes(t) ?? false}
